<script lang="ts">
  ...
  let { selectedPage = $bindable() }: Props = $props();
...
<ul class="menu h-full bg-base-200 w-56">
  ...
  <li>
    ...
    <ul>
      ...
      {@render button('persistence', 'Persistence')}
    </ul>
  </li>
  ...
  <li>
    ...
    <ul>
      ...
      {@render button('output', 'Output')}
      {@render button('request', 'Request')}
      {@render button('bonus', 'Bonus')}
    </ul>
  </li>
